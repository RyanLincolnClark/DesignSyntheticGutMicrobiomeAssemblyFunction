...
but_parameters=csvread('ER-FP-CC-RI_model.csv');
...
for communitysize=18:24
	mkdir(strcat(int2str(communitysize),'MemberComms/'));
	%Generate a matrix containing all possible combinations for a given
	%community size
	comms=nchoosek([1:15,17,19,20,24,25],communitysize-4);
	comms=[comms 16.*ones(size(comms,1),1) 21.*ones(size(comms,1),1) 22.*ones(size(comms,1),1) 23.*ones(size(comms,1),1)];
	comms=sort(comms,2);
	...
	parfor k=1:size(comms,1)
		...
		data=[]
		if sum(presentvector.*bpbvector)==4
			mystring='';
