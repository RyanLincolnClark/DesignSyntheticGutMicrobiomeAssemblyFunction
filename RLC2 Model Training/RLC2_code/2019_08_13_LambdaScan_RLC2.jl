...
lambdas=[10000,1000,100,10,1,0.1,0.01]
...
for initialguess=1:10

for l in lambdas
outputpath = "../2019_08_13_output_RLC2/lambda"*string(l)*"/guess"*string(initialguess)*"/lambda"*string(l)*"_guess"*string(initialguess)*"_"
...
args[:lambda]=l
#opts[:guesspath]="../2019_07_09_output_RLC1_MeanError_warmstart/lambda90/guess1/lambda90_guess1_"
...
#args[:y_start]=Dict((exp,i,ii,j,k)=>getvalue(outs[:m_children][exp][i][:y][ii,j,k])
...
#args[:r_start]=Dict((exp,i,ii,j,k)=>rand() for exp in keys(data) for i in 1:length(data[exp]) for ii in 1:length(data[exp][i]) for j in 1:data[exp][i][ii][:n_species] for k in 1:data[exp][i][ii][:n_disc_time])
