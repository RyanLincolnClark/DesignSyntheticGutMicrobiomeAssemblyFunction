function [complexities, initialconditions, timepoints, abundances] = LOAD_DATA_PERTURB_RLC(directory)
	global nspecies
	...
	for i=1:size(files,1)
		...
		for j=1:size(timepoints{i},2)
			for ii=1:complexities(i)
				rng('shuffle');
				newabundance(specieslist(ii),j)=abundance(ii,j)+normrnd(0,error(ii,j));
			end
		end
